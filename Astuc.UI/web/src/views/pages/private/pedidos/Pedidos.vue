<template>
    <div class="container mx-auto px-4">
        ...
        <div class="mt-8">
            ...
            <Table :headers="table.headers" :items="pedidosFiltrados" :actions="table.actions" @action="handleAction" />
            <!-- Asegúrate de importar el componente Edit adecuado para los insumos -->
            <Edit :visible="editVisible" :pedido="pedidoEditable" @update="handleUpdate" @close="handleClose" />
...
<script>
import Edit from '@/views/pages/private/pedidos/Edit.vue'; // Asegúrate de que la ruta sea correcta
...
export default {
    ...
    setup() {
        ...
        const handleUpdate = async (data) => {
            ...
            if (data.isNew) {
                await pedidoService.add(data)
                fetchPedidos()
            }
            else {
                await pedidoService.edit(data.id, data)
                fetchPedidos()
        ...
        const eliminarData = async (data) => {
            await pedidoService.remove(data)
            fetchPedidos()
        };

        const updateData = async (data) => {
            pedidoEditable.value = data;
        };
        ...
        const table = reactive({
            headers: ['Id', 'Cliente', 'Fecha de pedido', 'Estado', 'Total', "Direccion","Ciudad", "Código Postal","Celular"],
            actions: {
                edit: {
                    id: 'edit',
                    name: 'Edit',
                    icon: "fa fa-edit",
                    showName: false,
                    to: toUrl('/admin/pedidos/{id}/edit'),
                    danger: false
                },
                delete: {
                    id: 'delete',
        ...
        const handleAction = ({ actionId, item }) => {
            if (actionId.id === 'edit') {
                pedidoEditable.value = state.data.find(e => e.id == item.id);
                // pedidoEditable.value = item;
                state.editVisible = true;
            } else if (actionId.id === 'delete' && confirm('¿Estás seguro de que deseas eliminar estos datos?')) {
                eliminarData(item.id);
            }
            else if(actionId.id == "showDetails"){
